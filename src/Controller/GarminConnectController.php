...
class GarminConnectController extends AbstractController {
    ...
    #[Route('/garmin/connect/cookies', name: 'garmin_connect_cookies', methods: ['GET', 'POST'])]
    ...
    #[Route('/garmin/connect/update/last/data/sleep/refresh', name: 'garmin_connect_update_last_data_sleep_refresh', methods: ['POST'])]
    public function updateLastDataSleepRefresh(Request $request, Config $config): Response {
        $data = $request->getContent();
        $data_json = json_decode($data, true);
        $config->setConfigByKey('last_data_sleep_refresh', $data_json['timestamp']);

        return new Response('', Response::HTTP_ACCEPTED);
    }

    #[Route('/garmin/connect/sleep/data', name: 'garmin_connect_sleep_data')]
    public function getSleepDataFromGarminConnect(Request $request): Response {
        ...
        if(isset($response_json['dailySleepDTO'])) {
            $a = [
                'day' => $day,
                'startedAt' => $response_json['dailySleepDTO']['sleepStartTimestampGMT'] / 1000,
                'stoppedAt' => $response_json['dailySleepDTO']['sleepEndTimestampGMT'] / 1000
            ];
        } else {
            $a = $response_json;
        }
...
//        // $config->setConfigByKey('last_data_sleep_refresh', 1640777600);
//
